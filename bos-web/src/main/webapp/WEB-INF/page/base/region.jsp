...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath }/reginAction_findRegion.action",
			idField : 'id',
			columns : columns,
...
// 导入文件
       $("#button-import").upload({
           action:"${pageContext.request.contextPath }/reginAction_reginUpload.action",
           name:"reginFile",
           onComplete:function(data){
        	   var data = eval('(' + data + ')');  // change the JSON string to javascript object 
        	 
               if(data.success){
                   $.messager.alert("文件导入","文件导入成功！");
                   }else{ $.messager.alert("文件导入","文件导入失败！");}
